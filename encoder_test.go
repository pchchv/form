...
func TestEncoderFloat(t *testing.T) {
	...
	values, errs := encoder.Encode(test)
	assert.Equal(t, errs, nil)
	assert.Equal(t, len(values), 35)

	val, ok := values["Float32"]
	assert.Equal(t, ok, true)
	assert.Equal(t, val[0], "3.3")
	...
	assert.Equal(t, ok, true)
	assert.Equal(t, len(val), 3)
	assert.Equal(t, val[0], "1.1")
	assert.Equal(t, val[1], "2.2")
	assert.Equal(t, val[2], "3.3")
...
func TestEncoderBool(t *testing.T) {
	...
	assert.Equal(t, ok, true)
	assert.Equal(t, len(val), 3)
	assert.Equal(t, val[0], "false")
	assert.Equal(t, val[1], "true")
	assert.Equal(t, val[2], "true")
...
func TestEncoderStruct(t *testing.T) {
	...
	assert.Equal(t, errs, nil)
	assert.Equal(t, len(values), 16)

	val, ok := values["name"]
	assert.Equal(t, ok, true)
	assert.Equal(t, val[0], "joeybloggs")
	...
	assert.Equal(t, ok, true)
	assert.Equal(t, val[0], "1(111)111-1111")

	val, ok = values["Phone[1].Number"]
	assert.Equal(t, ok, true)
	assert.Equal(t, val[0], "9(999)999-9999")
	...
	val, ok = values["Time"]
	assert.Equal(t, ok, true)
	assert.Equal(t, val[0], tm.Format("2006-01-02"))
	...
	assert.Equal(t, ok, true)
	assert.Equal(t, val[0], "existingvalue")

	val, ok = values["mp[key]"]
	assert.Equal(t, ok, true)
	assert.Equal(t, val[0], "value")
	...
	val, ok = values["ArrayTime[0]"]
	assert.Equal(t, ok, true)
	assert.Equal(t, val[0], tm.Format(time.RFC3339))
}
...
func TestDecodeAllNonStructTypes(t *testing.T) {
	...
	assert.Equal(t, err, nil)
	assert.Equal(t, values[""][0], "2006-01-02T15:04:05Z")
	...
	assert.Equal(t, values[""][0], "arr1")
	assert.Equal(t, values[""][1], "arr2")
	...
	assert.Equal(t, err, nil)
	assert.Equal(t, len(values), 2)
	assert.Equal(t, values["[0]"][0], "arr1")
	assert.Equal(t, values["[1]"][0], "arr2")
	...
	assert.Equal(t, err, nil)
	assert.Equal(t, len(values), 2)
	assert.Equal(t, values["[key1]"][0], "val1")
	assert.Equal(t, values["[key2]"][0], "val2")
}
...
func TestEncoderNativeTime(t *testing.T) {
	...
	assert.Equal(t, ok, true)
	assert.Equal(t, val[0], "0001-01-01T00:00:00Z")
}
...
func TestEncoderErrors(t *testing.T) {
	...
	k = ee["Struct"]
	assert.Equal(t, k.Error(), "Unsupported Map Key '<struct {} Value>' Namespace 'Struct'")
}
...
func TestEncoderPanicsAndBadValues(t *testing.T) {
	...
	assert.Equal(t, values, nil)

	_, ok := err.(*InvalidEncodeError)
	assert.Equal(t, ok, true)
	assert.Equal(t, err.Error(), "form: Encode(nil)")
	...
	_, ok = err.(*InvalidEncodeError)
	assert.Equal(t, ok, true)
	assert.Equal(t, err.Error(), "form: Encode(nil *form.TestStruct)")
}
...
func TestEncoderExplicit(t *testing.T) {
	...
	assert.Equal(t, err, nil)
	assert.Equal(t, len(values), 1)
	assert.Equal(t, values["Name"][0], "Joeybloggs")
}
...
func TestEncoderEmbedModes(t *testing.T) {
	...
	assert.Equal(t, err, nil)
	assert.Equal(t, len(values), 1)
	assert.Equal(t, values["Field"][0], "B Val")
	assert.Equal(t, values["Field"][1], "A Val")
	...
	assert.Equal(t, len(values), 2)
	assert.Equal(t, values["Field"][0], "B Val")
	assert.Equal(t, values["A.Field"][0], "A Val")
}
...
func TestOmitEmpty(t *testing.T) {
	...
	assert.Equal(t, err, nil)
	assert.Equal(t, len(values), 1)
	assert.Equal(t, values["String"][0], "")
	...
	assert.Equal(t, err, nil)
	assert.Equal(t, len(values), 2)
	assert.Equal(t, values["x"][0], "0")
	assert.Equal(t, values["arr[0]"][0], "")
}
